...
public class GeneratePayrolls implements Command<PayrollBLDto> {
	...
	private void listContracts() throws BusinessException {
		// Listamos todos los contratos
		List<ContractDALDto> contracts = cg.findAll();
		// Si hay contratos
		if (!contracts.isEmpty()) {
			...
			for (ContractDALDto c : contracts) {
				if (c.state.equals(ContractState.IN_FORCE)
						|| c.state.equals(ContractState.TERMINATED)) {
	...
	private Optional<PayrollDALDto> findPayroll(List<PayrollDALDto> payrolls) {
		...
		for (PayrollDALDto p : payrolls) {
			if (p.date.getMonthValue() == this.date.getMonthValue()
