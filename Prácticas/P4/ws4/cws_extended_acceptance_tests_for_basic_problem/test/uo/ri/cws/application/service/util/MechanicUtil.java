...
import uo.ri.cws.application.business.mechanic.crud.commands.AddMechanic_TS;
import uo.ri.cws.application.business.mechanic.crud.commands.FindMechanicById_TS;
...
public class MechanicUtil {
	...
	public MechanicUtil loadById(String id) throws BusinessException {
		this.dto = new CommandExecutor().execute( new FindMechanicById_TS(id) )
				.orElse(null);
		return this;
	}
	...
	public MechanicUtil register() throws BusinessException {
		...
		dto.id = new CommandExecutor().execute( new AddMechanic_TS(this.dto) ).id;
		return this;
	}

	public MechanicUtil registerIfNew() throws BusinessException {
		Optional<MechanicBLDto> op = 
				new CommandExecutor().execute( new FindMechanicById_TS(dto.id) ); 
		if ( op.isEmpty() ) {
			register();
		}
		else {
