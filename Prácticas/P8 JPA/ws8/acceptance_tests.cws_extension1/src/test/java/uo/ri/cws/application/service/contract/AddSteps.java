...
public class AddSteps {
	...
	pay = trunc2Dec(pay);
	ContractDto contractDto = new ContractUtil().unique().forMechanic(m)
		.withType(ct).withGroup(pg).withAnnualWage(pay).get();
	...
	assertTrue(terminated.endDate.equals(
	...
	pay = trunc2Dec(pay); // (Math.round(pay * 100)) / 100;
	...
	settlement = trunc2Dec(settlement);
...
//	settlement = (double) settlement / 100;
	...
	if (numberOfEntireYearsWorked > 0) {
	    ...
	    settlement = numberOfEntireYearsWorked * dailyGrossWage
		    * compensationDays;
	    settlement = trunc2Dec(dailyGrossWage);
	}
	return settlement;
    }
