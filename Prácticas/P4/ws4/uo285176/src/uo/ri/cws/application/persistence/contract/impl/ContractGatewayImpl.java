...
public class ContractGatewayImpl implements ContractGateway {
	...
	public List<ContractDALDto> findAll() {
		...
		try {
			...
			pst = c.prepareStatement(Conf.getInstance()
					.getProperty("TCONTRACTS_findAll"));
			rs = pst.executeQuery();
	...
	public List<ContractDALDto> findContractsTerminated() {
		...
		try {
			...
			pst = c.prepareStatement(Conf.getInstance()
					.getProperty("TCONTRACTS_findContractsTerminated"));
			rs = pst.executeQuery();
			contracts = ContractAssembler.toContractListDALDto(rs);
		} catch (SQLException e) {
			throw new PersistenceException("Database error");// Esto hay que
	...
	@Override
	public List<ContractDALDto> findContractsByContractType(String type) {
		List<ContractDALDto> contracts = null;
		Connection c = null;
		PreparedStatement pst = null;
		ResultSet rs = null;
		...
		try {
			...
			pst = c.prepareStatement(Conf.getInstance()
					.getProperty("TCONTRACTS_findContractsByContractType"));
